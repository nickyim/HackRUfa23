...
const Submissions = () => {
  ...
  const handleSubmitResponse = (submission) => {
    if (currentResponse) {
      const newMessageRef = ref(db, `users/${submission.userId}/submissions/${submission.id}/chat`);
      const newMessage = {
        sender: "professional",
        timestamp: Date.now(),
        ...
        >
          <h4>
            ...
            {(submission.submission && typeof submission.submission === 'string') 
                ? submission.submission.substring(0, 30) + (submission.submission.length > 30 ? "..." : "") 
                : ''}
          </h4>
        </div>
      ))}
        ...
        <Modal.Body>
          {selectedSubmission && (
            <>
              ...
              <p>{selectedSubmission.submission || 'No submission provided.'}</p>
                {getChatAsArray(selectedSubmission.chat).sort((a, b) => a.timestamp - b.timestamp).map((chatMessage, chatIndex) => (
                    <div key={chatIndex} className={chatMessage.sender}>
                        ...
                        <small>
                        {chatMessage.sender === "user" ? selectedSubmission.username : "Professional"}
                        {" - "}
              ...
              <div>
                ...
                />
                <Button onClick={() => handleSubmitResponse(selectedSubmission)}>
                  Submit Response
                </Button>
              </div>
            </>
